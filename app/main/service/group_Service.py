...
def add_user(data):
    ...
    if user and group:
        group_user = Group_user.query.filter_by(public_id=get_public_id(
            data['email']), group_id_value=data['group_id']).first()
        if group_user:
            response_object = {
                'status': 'success',
...
def delete_user(data):
    public_id = get_public_id(data['email'])
    user = Group_user.query.filter_by(
        public_id=public_id, group_id_value=data['group_id']).first()
    if user:
